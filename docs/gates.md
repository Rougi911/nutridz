...
| 2026-06-14 | P4.11 bugs nutritionnels : USDA Branded, noms anglais CIQUAL, quantity_g non appliqué | GO | 118/118 tests, rankByDataType, callGemini lang param (ar→fr), resolveNutrition scaling + sel + estimated_portion, err.message masqué 422, sl-api.md type:"food" | Vision path non couvert — différé P5 |
| 2026-06-14 | P4.12 scoring CIQUAL forme brute + estimation portion réelle + termes génériques | GO | 164/164 tests. rawBonus/transformMalus (exemption conditionnelle généralisée à tous marqueurs, B-1 "poulet rôti" corrigé, allMatch boost multi-mots). defaultPortion() 15 règles. isGenericFoodTerm() + GENERIC_FOOD_TERMS. resolveNutrition(quantity_explicit) + portion_source. KO-1 disclaimer vision aligné. KO-2/KO-3 generateConseil rendu informatif (REG-04/REG-05). MAJ-1 log query USDA supprimé (RGPD). parseGeminiJSON 2e parse gardé. Math.max(0,...) dans score(). | — |
| 2026-06-14 | P4.13 meal_type + activité (MET) + scan/label (Gemini Vision) | GO | 198/198 tests. A1: mealTypeFromHour() + client_hour param (fuseau) + VALID_MEAL_TYPES (rejet "null" string). A2: MET_SIMPLE + estimateCaloriesBurned() (AL-02) + calories_burned_estimated. A3: POST /api/scan/label (callGeminiLabel + disclaimer + needs_confirmation). Revue-code: B-1 body limit 15mb /scan/label (server.js), B-2 image type+size validation, B-3 ECONNREFUSED guard. Réglementaire: KO-1 DISCLAIMER_REG05 tri-lingue /interpret, KO-2 DISCLAIMER_LABEL /scan/label, KO-3 DISCLAIMER trilingual deficiencies.js, KO-4 err.message masqué profile.js. Critique-algo: B-1 calcTDEE clés ASCII (sedentaire/modere), M-1 needs_confirmation /label. SL-03: Arabic Unicode escapes. | — |
