...
describe('G4-a — POST /api/activities/range', () => {
  test('agrège les calories brûlées par jour sur la plage', async () => {
    const today = new Date().toISOString().slice(0, 10);
    await createManual('u1', { type: 'course', duration_min: 30, date: today });
    const res = await request(app).post('/api/activities/range').set('x-test-user', 'u1').send({ days: 7 });
    expect(res.status).toBe(200);
    expect(Array.isArray(res.body)).toBe(true);
    const row = res.body.find((r) => r.date === today);
    expect(row).toBeTruthy();
