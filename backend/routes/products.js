...
router.get('/', (req, res) => {
  ...
  const total = db.prepare('SELECT COUNT(*) as n FROM products WHERE 1=1' + (q ? ' AND (name LIKE ? OR brand LIKE ?)' : '') + (category ? ' AND category = ?' : '')).get(...params.slice(0, -2)).n;
...
router.get('/categories', (req, res) => {
  const db = getDB();
  const rows = db.prepare('SELECT category, COUNT(*) as count FROM products GROUP BY category ORDER BY count DESC').all();
  res.json(rows);
});
...
router.get('/:id', (req, res) => {
  const db = getDB();
  const product = db.prepare('SELECT * FROM products WHERE id = ? OR barcode = ?').get(req.params.id, req.params.id);
