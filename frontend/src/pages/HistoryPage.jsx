  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 10, padding: '1rem 1.25rem 0' }}>
        {[
          { key: 'avgKcal', val: avgKcal + ' ' + t('common.kcal'), color: '#1A6B3C' },
          { key: 'daysOnTarget', val: `${daysOnTarget} / ${data.length}`, color: '#185FA5' }
        ].map(({ key, val, color }) => (
          <div key={key} style={{ background: '#fff', border: '0.5px solid rgba(0,0,0,0.08)', borderRadius: 12, padding: '12px 14px' }}>
            <div style={{ fontSize: 11, color: '#888' }}>{t(`history.${key}`)}</div>
      ...
      <div style={{ margin: '1rem 1.25rem 0', background: '#fff', border: '0.5px solid rgba(0,0,0,0.08)', borderRadius: 12, padding: '14px 16px' }}>
        <div style={{ fontSize: 13, fontWeight: 500, marginBottom: 12 }}>{t('history.caloriesPerDay')}</div>
        {data.length === 0 ? (
          <div style={{ textAlign: 'center', color: '#bbb', padding: '2rem', fontSize: 13 }}>{t('history.noData')}</div>
        ) : (
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={data} margin={{ top: 4, right: 4, bottom: 0, left: -20 }}>
              <XAxis dataKey="date" tick={{ fontSize: 11 }} axisLine={false} tickLine={false} />
              <YAxis tick={{ fontSize: 11 }} axisLine={false} tickLine={false} />
              <Tooltip formatter={(v) => [`${v} ${t('common.kcal')}`, t('history.calories')]} contentStyle={{ fontSize: 12, borderRadius: 8, border: '0.5px solid rgba(0,0,0,0.1)' }} />
              <ReferenceLine y={target} stroke="#BA7517" strokeDasharray="3 3" label={{ value: t('history.target'), position: 'right', fontSize: 10, fill: '#BA7517' }} />
      ...
      {data.length > 0 && (
        <div style={{ margin: '1rem 1.25rem 0', background: '#fff', border: '0.5px solid rgba(0,0,0,0.08)', borderRadius: 12, padding: '14px 16px' }}>
          <div style={{ fontSize: 13, fontWeight: 500, marginBottom: 12 }}>{t('history.macros')}</div>
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', marginTop: 8 }}>
            {[['#BA7517', 'glucides'], ['#185FA5', 'proteines'], ['#993C1D', 'lipides']].map(([c, k]) => (
              <div key={k} style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 11, color: '#666' }}>
                <div style={{ width: 10, height: 10, background: c, borderRadius: 2 }} />{t(`common.${k}`)}
              </div>
            ))}
          </div>
        </div>
