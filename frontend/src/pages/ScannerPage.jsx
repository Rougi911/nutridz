...
export default function ScannerPage() {
  ...
  const handleDetected = useCallback(async (code) => {
    ...
    try {
      ...
      toast.error('Erreur réseau lors de la recherche');
  ...
  const subtitles = {
    [PHASES.SCAN]:     'Pointez la caméra vers un code-barres EAN',
    [PHASES.LOADING]:  `Code détecté : ${lastCode}`,
    [PHASES.FOUND]:    'Produit identifié',
    [PHASES.NOTFOUND]: 'Produit non trouvé dans la base',
    [PHASES.ERROR]:    '',
  };
  ...
  return (
    <div style={{ minHeight: '100dvh', background: '#f7f7f5' }}>
      ...
      <div style={{ background: '#1A6B3C', color: 'white', padding: '1rem 1.25rem 1.5rem', borderRadius: '0 0 24px 24px' }}>
        <h1 style={{ fontSize: 22, fontWeight: 500 }}>📷 Scanner un produit</h1>
      ...
      <div style={{ padding: '1rem 1.25rem' }}>
        ...
        {phase === PHASES.LOADING && (
          <div style={{ textAlign: 'center', padding: '3rem 1rem' }}>
            ...
            <p style={{ marginTop: 12, color: '#555', fontWeight: 500 }}>Recherche du produit…</p>
        ...
        {phase === PHASES.FOUND && result?.product && (
          <ProductCard product={result.product} source={result.source} onDetails={() => navigate(`/products/${result.product.id}`)} onReset={reset} />
        )}

        {/* ── Produit introuvable → OCR ── */}
        {phase === PHASES.NOTFOUND && (
          <div style={{ background: '#fff', borderRadius: 16, padding: '1.5rem', textAlign: 'center', boxShadow: '0 2px 12px rgba(0,0,0,0.07)' }}>
            <div style={{ fontSize: 48 }}>🔎</div>
            <p style={{ fontWeight: 600, marginTop: 10, fontSize: 16 }}>Produit inconnu</p>
            <p style={{ fontSize: 12, color: '#aaa', marginTop: 4 }}>Code : {result?.barcode || lastCode}</p>
            <p style={{ fontSize: 13, color: '#555', marginTop: 10, lineHeight: 1.5 }}>
              Ce produit n'est pas encore dans la base.
              Scannez l'étiquette nutritionnelle pour l'identifier automatiquement.
            </p>
        ...
        {phase === PHASES.ERROR && (
          <div style={{ background: '#fff3f0', borderRadius: 16, padding: '1.5rem', textAlign: 'center' }}>
            ...
            <p style={{ color: '#993C1D', fontWeight: 500, marginTop: 10, fontSize: 14 }}>
              {camError || 'Erreur lors de la recherche du produit'}
            </p>
...
function ProductCard({ product: p, source, onDetails, onReset }) {
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 16, padding: '1.25rem', boxShadow: '0 2px 12px rgba(0,0,0,0.07)' }}>
      <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 11, color: '#888', marginTop: 3 }}>
            {source === 'local' ? '📦 Base NutriDZ' : '🌍 OpenFoodFacts'}
          </div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8, marginTop: 16 }}>
        {[
          { label: 'Kcal',     val: p.kcal_per100 },
          { label: 'Glucides', val: p.per100?.glucides },
          { label: 'Protéines',val: p.per100?.proteines },
          { label: 'Lipides',  val: p.per100?.lipides },
        ].map(({ label, val }) => (
          <div key={label} style={{ background: '#f7f7f5', borderRadius: 10, padding: '8px 4px', textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 9, color: '#aaa', marginTop: 1 }}>/ 100g</div>
      ...
      <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
        <button onClick={onDetails} style={btnStyle('#1A6B3C', '#fff')}>Voir détails</button>
        <button onClick={onReset}   style={btnStyle('#f0f0ec', '#555')}>Scanner autre</button>
      </div>
    </div>
  );
}
